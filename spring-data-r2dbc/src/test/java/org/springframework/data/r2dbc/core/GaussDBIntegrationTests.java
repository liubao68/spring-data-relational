...
import org.springframework.data.r2dbc.dialect.GaussDBDialect;
import org.springframework.data.r2dbc.testing.ExternalDatabase;
import org.springframework.data.r2dbc.testing.GaussDBTestSupport;
...
import io.r2dbc.gaussdb.codec.Interval;
...
/**
 * Integration tests for GaussDB-specific features such as array support.
 *
 * Notes: this file is token from PostgresIntegrationTests and add specific changes for GaussDB
 *
 * @author liubao
 */
public class GaussDBIntegrationTests extends R2dbcIntegrationTestSupport {

	@RegisterExtension public static final ExternalDatabase database = GaussDBTestSupport.database();

	private final DataSource dataSource = GaussDBTestSupport.createDataSource(database);
	private final ConnectionFactory connectionFactory = GaussDBTestSupport.createConnectionFactory(database);
	...
	void shouldWriteAndReadEnumValuesUsingDriverInternals() {
		...
		ReactiveDataAccessStrategy strategy = new DefaultReactiveDataAccessStrategy(GaussDBDialect.INSTANCE,
	...
	void shouldReadAndWriteGeoTypes() {
		...
		R2dbcEntityTemplate template = new R2dbcEntityTemplate(client,
				new DefaultReactiveDataAccessStrategy(GaussDBDialect.INSTANCE));
